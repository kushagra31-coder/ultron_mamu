...
- **Early-commit (Jev-style preemptive execution)** — while you are still speaking, the STT service transcribes the tail of the audio every ~1.5 s and streams partials to the agent's `/partial` gate. If a partial is a confident closed-set command (opening an allowlisted app — e.g. "open notepad", "launch calculator"), it executes **immediately**, before the utterance finishes. The gate is pure regex (zero model calls, zero extra VRAM), fires once per command per utterance, drops stale partials, and never touches free text, dictation, file writes, or anything destructive. Say "no" / "never mind" / "undo" to reverse the last early action (it closes the window it opened). The finished utterance still runs the normal pipeline, which is told what already happened so it never repeats it. Tune with `ULTRON_EARLY_COMMIT=0` (disable), `ULTRON_PARTIAL_INTERVAL` (seconds), `ULTRON_PARTIAL_WINDOW` (seconds of tail audio per partial).
